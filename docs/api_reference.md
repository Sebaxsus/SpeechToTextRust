...
- **`cargo run -- --cors-allow-all`** (agregado 2026-08-06, `router::cors_layer`/`cors_layer_permisivo`): reemplaza el `CorsLayer` de origen único de arriba por uno que acepta cualquier origen (`Access-Control-Allow-Origin: *`), útil para consultar la API desde cualquier dispositivo de la LAN sin fijar `CLIENT_ORIGIN`. Se loguea un `tracing::warn!` al construir el router recordando que esto no reemplaza `MCP_BEARER_TOKEN` — sigue siendo el único control de acceso real de los endpoints protegidos, este flag solo relaja el chequeo de origen del *browser*. Mismo patrón de lectura de CLI que `--log`/`--greedy`: `std::env::args()` leído directamente, sin pasar por `Config`/`.env`.
